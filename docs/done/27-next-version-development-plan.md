...
本文档用于把当前开发进度和本地优化方向收敛成一个适合立即推进的版本计划。`优化方向.md` 和 `UI优化方向.md` 作为本地参考资料；其中 UI 优化方向暂不进入本版本。

V8 及后续版本的完整路线见 [V8+ Agent 详细开发路线](28-v8-plus-agent-development-roadmap.md)。V9 及以后执行级计划见 [V9+ 后续开发规划](31-v9-plus-development-roadmap.md)。本文保留 V8.1 的计划拆分和完成标准。

执行状态：V8.1 已完成第一版，验收结果见 [V8.1 验收记录](29-v8-acceptance-notes.md)。V8.2/V8.3 也已完成第一版，验收结果见 [V8.2/V8.3 验收记录](30-v8-2-v8-3-acceptance-notes.md)。
...
- V8.1 工作目录文件 Agent MVP：工作目录文件服务、`workspace.*` 工具、Agent 文件步骤执行、连续执行 MVP 和提示词注入防护。

V9 MVP、V9.1、V9.2、V10.1、V10.2 和 V10.3 第一版已完成，后续还缺：

- 命令失败后的自动重规划。
- tool result 回传后的自动重规划。
- 工具生态扩展、自动重规划、电脑感知和设备输入模拟。
...
- 不依赖真实 AI Key 即可自动化测试。
- 不依赖 UI 大改。
- 核心边界可以用临时目录稳定验证。
- 是后续 Agentic Loop 和受控命令执行的基础。
...
当前 V8.2/V8.3、V9 MVP 和 V9.1 均已完成第一版，后续实际执行应参考 [V9+ 后续开发规划](31-v9-plus-development-roadmap.md)。
