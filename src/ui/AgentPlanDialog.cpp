...
void AgentPlanDialog::executeRemainingSteps()
{
    ...
    while (!m_stopRequested) {
        if (timer.elapsed() >= MaxContinuousExecutionMs) {
            runtimeLimitReached = true;
            break;
        }

        const int index = nextExecutableStepIndex();
        if (index < 0) {
            break;
        }

        if (executedStepCount >= MaxContinuousExecutionSteps) {
