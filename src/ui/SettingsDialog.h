...
private:
    ...
    // 功能：控制 Python sidecar 启动参数显隐；使用模块：后端下拉框切换。
    ...
    QLabel *m_backendLabel = nullptr;       // 功能：AI 后端字段标签；使用模块：applyTexts。
    ...
    QLabel *m_pythonExecutableLabel = nullptr; // 功能：Python 命令标签；使用模块：sidecar 后端。
    QLabel *m_pythonSidecarLabel = nullptr; // 功能：Python sidecar 目录标签；使用模块：sidecar 后端。
    ...
    QComboBox *m_backendCombo = nullptr;    // 功能：AI 调用后端选择；使用模块：生成 AppConfig。
    ...
    QLineEdit *m_pythonExecutableEdit = nullptr; // 功能：Python 命令输入；使用模块：生成 AppConfig。
    QLineEdit *m_pythonSidecarEdit = nullptr; // 功能：sidecar 目录输入；使用模块：生成 AppConfig。
